...
void c_renderer::DrawCircle3D(D3DXVECTOR3 vPos, float flPoints, float flRadius, ImColor clrColor, float flThickness)
{
	...
	for (float flAngle = 0; flAngle < (D3DX_PI * 2.0f); flAngle += flPoint)
	{
		...
		D3DXVECTOR3 vStart2 = D3DXVECTOR3(vStart.x, vStart.z, vStart.y);
		D3DXVECTOR3	vEnd2 = D3DXVECTOR3(vEnd.x, vEnd.z, vEnd.y);

		D3DXVECTOR3 vStartScreen = Engine::World2Screen(vStart2);
		D3DXVECTOR3 vEndScreen = Engine::World2Screen(vEnd2);

		render.draw_line(vStartScreen.x, vStartScreen.y, vEndScreen.x, vEndScreen.y, clrColor, flThickness);
		//Render->DrawLine(vStartScreen.x, vStartScreen.y, vEndScreen.x, vEndScreen.y, clrColor, flThickness);
	}
}